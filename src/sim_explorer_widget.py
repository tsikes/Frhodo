...
class SIM_Explorer_Widgets(QtCore.QObject):
    ...
    def create_choices(self, axis):
        ...
        itemMemory = {}
        main_parameter_box.info = {'axis': axis, 'boxes': self.widget[-1], 'itemMemory': itemMemory}
        sub_parameter_box.info =  {'axis': axis, 'boxes': self.widget[-1], 'itemMemory': itemMemory}
    ...
    def main_parameter_changed(self, event):
        ...
        for n in prior['checked']:
            subComboBox.model().item(n, 0).setCheckState(QtCore.Qt.Checked)    # check prior selected
    ...
    def sub_item_changed(self, sender=None):
        ...
        if added.size > 0:
            if len(comboBox.checked) > self.max_history:  # if length is greater than max, delete value
                n = 0
                if 'total' in sub_type and 0 == comboBox.checked[0]: # if total selected, delete next
                    n = 1
                
                item = comboBox.model().item(comboBox.checked[n], 0)
                comboBox.model().blockSignals(True)         # block update if checkstate changed
                item.setCheckState(QtCore.Qt.Unchecked)     # uncheck first non Total selection
    ...
    def update_plot(self, SIM=None):
        if self.updating_boxes: return  # do not update plot if updating boxes
        if not self.parent.mech_loaded: return # if mech isn't loaded successfully, exit
        ...
        for n, axis in enumerate(['x', 'y', 'y2']):
            ...
            if main_choice != '-':
                ...
                if self.widget[n][1].count() > 0:   # find all choices other than the selected index
                    sub_choice = sub_choice[sub_choice != selected_sub_choice].astype(int)
                    sub_choice = np.append(selected_sub_choice, np.sort(sub_choice))    # append list to selected
                else:
                    sub_choice = [0]      # for types with no sub_choices, default to first array
                
                if sub_type is None:        # if no Sub_type, do something
                    pass
                elif 'total' in sub_type:   # if total in subtype, subtract 1 (total is 0)
                    sub_choice = np.subtract(sub_choice, 1)
                
                raw_data = getData(SIM, SIM_name)
                if raw_data.ndim == 1:  # if raw data is a vector only, give it a second dimension
                    raw_data = raw_data[None, :]    # this is for times, velocity, temperature
                ...
                if len(itemList) > 0:
                    if SIM_name == 'delta_h':
                        labels = []
                        for n, choice in enumerate(sub_choice): # if delta enthalpy, add t(0) value in e format
                            ...
                            labels.append('{:s}:  dH(0) = {:s},  {:s}'.format(token[0], val, token[1]))
                        ...
                        for n, choice in enumerate(sub_choice): # if delta enthalpy, add t(0) value in e format
                            ...
                            # val = '{:>10.3e}'.format(data[axis][n][0])
                            # val = re.sub("e(-?)0*(\d+)", r"e\1\2", val.replace("e+", "e"))
                            labels.append('{:s}:  kr(0) = {:s},  {:s}'.format(token[0], val, token[1]))        
